...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("role", postgresql.ENUM("viewer", "editor", "admin", name="user_role",
                                          create_type=False), nullable=False, server_default="editor"),
        ...
        sa.CheckConstraint("LENGTH(username) BETWEEN 3 AND 30", name="users_username_len"),
        sa.CheckConstraint("username ~ '^[a-zA-Z0-9_]+$'", name="users_username_alnum"),
    )
    op.create_index("users_email_uk", "users",
    ...
    op.create_index("idx_evt_expires_at", "email_verification_tokens", ["expires_at"],
    ...
    op.create_index("idx_rt_active", "refresh_tokens", ["user_id"],
    ...
    op.create_table(
        ...
        sa.Column("user_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False),
        sa.Column("role", postgresql.ENUM("owner", "editor", "viewer", name="member_role",
    ...
    op.create_index("idx_pm_active", "project_members", ["project_id", "user_id"],
    ...
    op.create_table(
        ...
        sa.Column("invited_by", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("users.id"), nullable=False),
        sa.Column("status", postgresql.ENUM("pending", "accepted", "rejected", "expired",
    ...
    op.create_table(
        ...
        sa.Column("priority", postgresql.ENUM("low", "medium", "high", name="task_priority",
                                               create_type=False), nullable=False),
        sa.Column("status", postgresql.ENUM("abierto", "en_proceso", "completo",
    ...
    op.create_index("idx_tasks_project_id", "tasks", ["project_id"],
                    postgresql_where=sa.text("deleted_at IS NULL"))
    op.create_index("idx_tasks_status", "tasks", ["project_id", "status"],
                    postgresql_where=sa.text("deleted_at IS NULL"))
    op.execute("CREATE INDEX idx_tasks_search_vector ON tasks USING GIN (search_vector)")
    ...
    op.create_index("idx_tte_active", "task_time_entries", ["user_id"],
    ...
    op.create_index("idx_tc_task_id", "task_comments", ["task_id"],
                    postgresql_where=sa.text("deleted_at IS NULL"))
    ...
    op.create_table(
        ...
        sa.Column("comment_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("task_comments.id", ondelete="CASCADE"), nullable=False),
        sa.Column("mention_type", postgresql.ENUM("user", "task", name="mention_type",
                                                   create_type=False), nullable=False),
        sa.Column("mentioned_user_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=True),
        sa.Column("mentioned_task_id", postgresql.UUID(as_uuid=True),
    ...
    op.execute("CREATE INDEX idx_al_created_at_brin ON audit_logs USING BRIN (created_at)")
...
def downgrade() -> None:
    ...
    op.execute("DROP TRIGGER IF EXISTS trg_task_assignees_timer_disabled ON task_assignees")
    ...
    for table in [
        ...
        "projects", "system_settings", "password_reset_tokens",
        "refresh_tokens", "email_verification_tokens", "audit_logs", "users",
    ]:
        op.drop_table(table)
