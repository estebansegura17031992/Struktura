...
# ─────────────────────────────────────────────────────────────────────────────
# Frontend Sprint 4 · Objetivo 6 — GET /projects/{id}/invitations
# (endpoint agregado junto con la UI de gestión de invitaciones pendientes;
# no existía hasta este sprint — ver invitation_service.py::list_invitations)
# ─────────────────────────────────────────────────────────────────────────────


@pytest.mark.asyncio
async def test_owner_puede_listar_invitaciones(client: AsyncClient, db_session):
    await _register_and_verify(client, "lowner@test.dev", "lowner")
    # Forzar rol global no-admin explícito — el primer usuario registrado en
    # DB limpia es admin automáticamente, y queremos probar el gate de
    # membresía (owner), no el bypass de admin (ver test_lista_admin abajo).
    await _set_role(db_session, "lowner@test.dev", "editor")
    owner_headers = await _headers(client, "lowner@test.dev")
    project = await _create_project(client, owner_headers)

    invite_resp = await _invite(
        client, owner_headers, project["id"], "listado@test.dev", "viewer"
    )
    assert invite_resp.status_code == 201

    resp = await client.get(
        f"/api/v1/projects/{project['id']}/invitations", headers=owner_headers
    )
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert len(body) == 1
    assert body[0]["email"] == "listado@test.dev"


@pytest.mark.asyncio
async def test_admin_puede_listar_sin_ser_miembro(client: AsyncClient, db_session):
    # Primer usuario registrado en DB limpia es admin automáticamente.
    admin = await _register_and_verify(client, "ladmin@test.dev", "ladmin")
    admin_headers = await _headers(client, admin["email"])

    owner = await _register_and_verify(client, "lowner2@test.dev", "lowner2")
    owner_headers = await _headers(client, owner["email"])
    project = await _create_project(client, owner_headers)

    invite_resp = await _invite(
        client, owner_headers, project["id"], "listado2@test.dev", "viewer"
    )
    assert invite_resp.status_code == 201

    # El admin NUNCA fue agregado como miembro del proyecto — solo tiene el
    # bypass de rol global.
    resp = await client.get(
        f"/api/v1/projects/{project['id']}/invitations", headers=admin_headers
    )
    assert resp.status_code == 200, resp.text
...
@pytest.mark.asyncio
async def test_editor_no_puede_listar_invitaciones(client: AsyncClient, db_session):
    await _register_and_verify(client, "lowner3@test.dev", "lowner3")
    await _set_role(db_session, "lowner3@test.dev", "editor")
    owner_headers = await _headers(client, "lowner3@test.dev")
    project = await _create_project(client, owner_headers)

    await _register_and_verify(client, "leditor@test.dev", "leditor")
    editor_id = (
        await client.get(
            "/api/v1/users/me", headers=await _headers(client, "leditor@test.dev")
        )
    ).json()["id"]
    await client.post(
        f"/api/v1/projects/{project['id']}/members",
    ...
    editor_headers = await _headers(client, "leditor@test.dev")

    resp = await client.get(
        f"/api/v1/projects/{project['id']}/invitations", headers=editor_headers
    )
    assert resp.status_code == 403


@pytest.mark.asyncio
async def test_viewer_no_puede_listar_invitaciones(client: AsyncClient, db_session):
    await _register_and_verify(client, "lowner4@test.dev", "lowner4")
    await _set_role(db_session, "lowner4@test.dev", "editor")
    owner_headers = await _headers(client, "lowner4@test.dev")
    project = await _create_project(client, owner_headers)

    await _register_and_verify(client, "lviewer@test.dev", "lviewer")
    viewer_id = (
        await client.get(
            "/api/v1/users/me", headers=await _headers(client, "lviewer@test.dev")
        )
    ).json()["id"]
    await client.post(
        f"/api/v1/projects/{project['id']}/members",
    ...
    viewer_headers = await _headers(client, "lviewer@test.dev")

    resp = await client.get(
        f"/api/v1/projects/{project['id']}/invitations", headers=viewer_headers
    )
    assert resp.status_code == 403


@pytest.mark.asyncio
async def test_lista_incluye_invitaciones_en_cualquier_estado(
    client: AsyncClient, db_session
):
    await _register_and_verify(client, "lowner5@test.dev", "lowner5")
    await _set_role(db_session, "lowner5@test.dev", "editor")
    owner_headers = await _headers(client, "lowner5@test.dev")
    project = await _create_project(client, owner_headers)

    # pending
    pending_resp = await _invite(
        client, owner_headers, project["id"], "l_pending@test.dev", "viewer"
    )
    assert pending_resp.status_code == 201

    # accepted
    accepted_token = await _invite_and_get_raw_token(
        client, owner_headers, project["id"], "l_accepted@test.dev", "viewer"
    )
    await _register_and_verify(client, "l_accepted@test.dev", "l_accepted_usr")
    accept_resp = await client.post(
        f"/api/v1/invitations/{accepted_token}/accept",
        headers=await _headers(client, "l_accepted@test.dev"),
    )
    assert accept_resp.status_code == 200

    # rejected
    rejected_token = await _invite_and_get_raw_token(
        client, owner_headers, project["id"], "l_rejected@test.dev", "viewer"
    )
    await _register_and_verify(client, "l_rejected@test.dev", "l_rejected_usr")
    reject_resp = await client.post(
        f"/api/v1/invitations/{rejected_token}/reject",
        headers=await _headers(client, "l_rejected@test.dev"),
    )
    assert reject_resp.status_code == 200

    # expired
    await _invite_and_get_raw_token(
        client, owner_headers, project["id"], "l_expired@test.dev", "viewer"
    )
    await db_session.execute(
        update(ProjectInvitation)
        .where(ProjectInvitation.email == "l_expired@test.dev")
        .values(expires_at=datetime.now(UTC) - timedelta(days=1))
    )
    await db_session.commit()
    service = InvitationService(db_session)
    ...
    assert resp.status_code == 200, resp.text
    body = resp.json()
    statuses = {item["email"]: item["status"] for item in body}
    assert len(body) == 4
    assert statuses["l_pending@test.dev"] == "pending"
    assert statuses["l_accepted@test.dev"] == "accepted"
    assert statuses["l_rejected@test.dev"] == "rejected"
    assert statuses["l_expired@test.dev"] == "expired"


@pytest.mark.asyncio
async def test_lista_vacia_si_no_hay_invitaciones(client: AsyncClient, db_session):
    await _register_and_verify(client, "lowner6@test.dev", "lowner6")
    await _set_role(db_session, "lowner6@test.dev", "editor")
    owner_headers = await _headers(client, "lowner6@test.dev")
    project = await _create_project(client, owner_headers)
